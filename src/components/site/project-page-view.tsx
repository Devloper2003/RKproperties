...
import {
  ...
  ChevronLeft, ChevronRight, Star, Sparkles, TrendingUp, Home, Eye,
} from "lucide-react";
  ...
  return (
    <AnimatePresence>
      {projectPageSlug && (
        ...
        >
          {/* Sticky top bar */}
          <div className="sticky top-0 z-50 bg-cream/95 backdrop-blur-md border-b border-gold/15 px-4 sm:px-6 py-3 flex items-center justify-between shadow-sm">
            <button
            ...
            {project && (
              <div className="hidden md:flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span className="font-medium text-indigo-deep">{project.name}</span>
                <span>·</span>
            ...
            <div className="flex items-center gap-2">
              <Button
                onClick={() => toast.success("🔗 Link copied! Share with friends.")}
                size="sm"
                variant="ghost"
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10">
                  <div className="max-w-4xl">
                    ...
                    <motion.h1
                      ...
                      className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-cream mb-2"
                    >
                      {project.name}
                    </motion.h1>
                    <motion.p
                      ...
                      className="text-gold italic text-lg sm:text-xl"
                    >
                      {project.tagline}
                    </motion.p>
                  </div>
              ...
              <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                ...
                >
                  {[
                    ...
                    { icon: LayoutGrid, label: "Plot Sizes", value: `${project.minPlotSize}-${project.maxPlotSize}`, sub: "sq.yd", color: "text-green-deep", bg: "bg-green-light/10" },
                    ...
                    >
                      ...
                      <div className="font-display font-bold text-indigo-deep text-sm mt-1">
                        {stat.value} {stat.sub && <span className="text-[10px] text-muted-foreground">{stat.sub}</span>}
                      </div>
                    </motion.div>
                  ))}
                </motion.div>
                ...
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="card-luxury-dark rounded-2xl bg-spiritual-temple p-6"
                >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                      {project.amenities.map((a: string, i: number) => (
                        <motion.div
                          ...
                          className="flex items-center gap-2 p-3 rounded-lg bg-marble text-sm text-indigo-deep"
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <Button
                    onClick={() => openLeadForm(project.id)}
                    className="gold-shimmer bg-gradient-to-br from-gold-light to-gold text-indigo-deep font-semibold h-12"
                  >
                    <MessageCircle className="w-4 h-4 mr-2" /> Book Site Visit
                  </Button>
                  <Button
                    onClick={() => { closeProjectPage(); setTimeout(() => openVisit(project.id), 100); }}
                    variant="outline"
                    className="border-gold/40 text-indigo-deep hover:bg-gold/10 h-12"
                  >
                  ...
                  <Button
                    asChild
                    variant="outline"
                    className="border-green-deep/40 text-green-deep hover:bg-green-light/10 h-12"
                  >
